			...
			<div className="flex-1 overflow-hidden">
				<div className="h-full overflow-y-auto p-6">
					<div className="bg-white rounded-lg shadow p-4">
						<h2 className="text-lg font-semibold mb-2">Automation preview</h2>
						<div className="text-sm text-gray-700">
							<div className="mb-2"><span className="font-medium">Slot:</span> {slot} – {CONTENT_GUIDELINES[slot]?.type}</div>
							<div className="mb-2"><span className="font-medium">Category:</span> {category || '—'}</div>
							<div className="mb-2"><span className="font-medium">Suggested topics:</span> {suggestedTopics.slice(0, 3).join(' | ') || '—'}</div>
							<pre className="bg-gray-50 border rounded p-3 overflow-auto text-xs">{JSON.stringify({
								config: { category, tone, audience, keywords: keywords.split(',').map((k) => k.trim()).filter(Boolean), outline: outline.split('\n').map((l) => l.trim()).filter(Boolean), guidelines: instructions, slot },
								schedule: { enabled, frequency, dayOfWeek, timeLocal, timezone },
							}, null, 2)}</pre>
						</div>
					</div>
				</div>
